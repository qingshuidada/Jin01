$(function(){
	/*内容高度*/
	$('#currentReport .listBody').css('height',$('#loadarea').height()-62 + 'px');
	...
	$('#currentReport .filtCondition .currentDate').html(new Date().Format('yyyy年MM月dd日'));
	$('#currentReport .filtCondition input[name=reportFormsDate]').val(time);
	//点击查询
	$('#currentReport .filtCondition .query').click(function(){		
		var reportFormsDate = $('#currentReport .filtCondition input[name=reportFormsDate]').val();
		...
		$('#currentReport .filtCondition .currentDate').html(currentDate);
		currentReportGetData(reportFormsDate,number);
	})
	
	$('#currentReport .outPut .write').click(function(){
		var reportFormsDate = $('#currentReport .filtCondition input[name=reportFormsDate]').val();
		excelplugin.submit({
			action:'../../ErpStatistics/writeEcoWorkshopTabToExcel.do?getMs='+getMS(),
			array:[
			       {name:'reportFormsDate',value:reportFormsDate}
			       ],
			type:'post'
		})
	})
...
function currentReportGetData(reportFormsDate,number){
	...
	$.ajax({
		url:'../../ErpStatistics/getEcoWorkshopTabData.do?getMs='+getMS(),
		data:{
	    ...
	    success:function(data){
	    	...
	    	if(data){
		    	...
		    	$('#currentReport .rigthH').show();
	    	}else{
	    		$('#currentReport .rigthH').hide();
	    	}
