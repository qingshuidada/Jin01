...
@Service
public class ErpSelectService extends BaseService{

	@Autowired
	private SqlSessionMap sqlSessionMap;
	@Autowired
	 ...
	 * 获得ErpSelectDao实例
	 */
	public ErpSelectDao getErpSelectDao(ErpSelectForm erpSelectForm){
		SqlSession sqlSession = sqlSessionMap.getSqlSession(erpSelectForm.getDataSourceKey());
		ErpSelectDao erpSelectDao = sqlSession.getMapper(ErpSelectDao.class);
		return erpSelectDao;
	}
	/**
	 * 白胚入库明细
	 */
	public PageModel<ErpSelectForm> queryVbprkmx(ErpSelectForm erpSelectForm){
		...
		ErpSelectDao erpSelectDao = getErpSelectDao(erpSelectForm);
		System.out.println("ssss="+erpSelectForm.getKehuFlag()+",yyyy="+erpSelectForm.getYwmanFlag());
